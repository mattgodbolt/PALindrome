...
- **More than 12 deposit threads.** Lane sweep of `--deposit-threads N` on
  ...
  12 against 16 sits within a few percent either way, but CPU climbs with
  every lane added (24 lanes burns 30% more than 12 for no wall at all): the
  fanned-out deposit hitting the memory system (#61 below) and dropping
  Skylake-X's all-core turbo, and that CPU is the margin a real-time source
  and the MJPEG encoder live on. So the default is 12. The earlier "16 is
  slower than 8" reading was true of that build and is not now; the reason
  not to go wider is CPU, not wall.
...
- **The decode thread** (still the wall on the file renders; on the live paths
  ...
  deliberately not bundled into #63; and the event-driven pass-3 restructure
  sketched on #63 (unify the gate replay with event detection, run the flow
  math branchless over constant-control runs) - worth scoping only after the
  limiter picture is re-measured. The U/V low-pass pair shares *taps* but not
  input, so the #62 fusion shape does not transfer; its sharable resource is
  only the tap broadcast (~10% by the same port math - probably not worth the
  surface).
