...
int RenderCommand::run() const {
  ...
  std::println("{} ({}x{}); envelope @ {:g} MS/s after /{} decimation, carrier {:.4f} MHz (AFC {:+.2f} kHz); "
               "horizontal {} {} edges @ {:.1f} Hz ({:+.2f}%); vertical locked {} fields @ {:.2f} Hz ({:+.2f}%)",
      what, width_, height_, envelope_rate / 1e6, decimate, es.carrier_hz / 1e6, es.afc_offset_hz / 1e3,
