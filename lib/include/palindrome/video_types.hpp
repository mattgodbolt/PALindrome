...
// [0, 1) (0 at the locked field start). The screen keys its per-field work on
// the free-running v_phase wrap (the retrace happens with or without lock). A
// second timing rail off the same sync bit the separator produces — it joins
// the horizontal rail at the renderer.
struct VSample {
  float v_phase = 0.0f;
};
