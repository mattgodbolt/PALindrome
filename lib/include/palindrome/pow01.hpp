...
// log2(|x|) for x in [0, 1]. Exact 0 at x == 1; x == 0 returns a huge finite
// negative (not the bit-pattern's -127, which a small enough exponent could
// carry past pow01's clamp and lift exact black off zero). Subnormal x reads
// as roughly -127 instead of its true log; pow01's range clamp absorbs the
// difference for any exponent above ~1/16. The sign strip is load-bearing:
// -0.0 passes a [0, 1] clamp (it compares equal to 0) but its sign bit would
// otherwise ride into the exponent and read as +129, turning black into full
// white.
[[nodiscard]] inline float log2_poly(float x) noexcept {
  const auto xi = std::bit_cast<std::uint32_t>(x) & 0x7FFFFFFFu;
  // A select, not a branch, so the loop stays vector.
  const float zero_log = xi == 0 ? -1e30f : 0.0f;
  const auto ex = static_cast<float>(static_cast<int>(xi >> 23) - 127) + zero_log;
...
// x^e for x in [0, 1] and e > 0. Exact 1 at x == 1; x == 0 returns 2^-40 for
// every e, as does x below ~2^(-40/e) - either way an 8-bit readout rounds
// it to 0.
