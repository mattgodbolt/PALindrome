...
TEST_CASE("pow01 endpoints are exact") {
  ...
  const auto e = GENERATE(1.0f / 25.0f, 1.0f / 2.2f, 1.0f / 2.6f, 0.5f, 1.0f, 2.2f);
...
TEST_CASE("pow01 reads tiny and subnormal inputs as black") {
  ...
  const auto e = GENERATE(1.0f / 2.8f, 1.0f / 2.2f, 1.0f);
  const auto x = GENERATE(0x1p-126f, // smallest normal
      0x1p-149f, // smallest subnormal
      0x1p-60f); // below any readout LSB
  CHECK(as_byte_kernel(pow01(x, e)) == 0);
}
...
TEST_CASE("pow01 stays within one 8-bit LSB of double pow across the unit interval") {
  ...
  const auto e = GENERATE(1.0f / 2.8f, 1.0f / 2.2f, 1.0f / 1.4f, 1.0f, 2.2f, 8.0f);
  int worst = 0;
